...
class EventDataVisitor implements IResourceDeltaVisitor {
	...
	private EventData findDeleteEventPairForRename(IResourceDelta delta,
			ChangeType type) {
		EventData data = null;
		// check if this is already in the list
		for (EventData event : events) {
			if (delta.getFullPath().toString().equals(event.movedFromPath)
					&& event.type == type) {
				data = event;
				break;
			}
	...
	private EventData findAddEventPairForRename(IResourceDelta delta,
			ChangeType type) {
		EventData data = null;
		// check if this is already in the list
		for (EventData event : events) {
			if (delta.getFullPath().toString().equals(event.movedToPath)
					&& event.type == type) {
				data = event;
				break;
			}
	...
	private void createPaths(EventData data, IResourceDelta delta) {
		if (delta.getResource() != null
				&& delta.getResource().getLocation() != null) {
